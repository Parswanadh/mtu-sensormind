...
# SQL Helper for MRO History
def get_service_history(engine_id):
    try:
        db_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'mro_assets.db')
        if not os.path.exists(db_path):
            return "Asset database not found. Run utils/db_setup.py."
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT last_maintenance_action, technician_notes FROM mro_history WHERE engine_id = ?", (int(engine_id),))
        row = cursor.fetchone()
        conn.close()
        if row:
            return f"{row[0]} | {row[1]}"
        return "Standard routine maintenance. No major overhaul history recorded."
    except Exception as e:
        return f"Service history unavailable: {str(e)}"
...
if 'selected_engine' not in st.session_state:
    st.session_state.selected_engine = 1
if 'current_cycle' not in st.session_state:
    st.session_state.current_cycle = 30 # Minimum required window
if 'agent_trace' not in st.session_state:
    st.session_state.agent_trace = []
if 'generated_wos' not in st.session_state:
    st.session_state.generated_wos = []
...
with st.sidebar:
    ...
    st.session_state.selected_engine = st.selectbox(
        "Select Engine ID", 
        engine_list, 
        index=int(np.where(engine_list == st.session_state.selected_engine)[0][0])
    )
    
    max_cycle = df[df['engine_id'] == st.session_state.selected_engine]['cycle'].max()
    
    st.session_state.current_cycle = st.slider(
        "Simulation Cycle (Time)", 
        min_value=30, 
        max_value=int(max_cycle), 
        value=min(st.session_state.current_cycle, int(max_cycle))
    )
    ...
    if st.button("🎤 Record Query"):
        with st.spinner("Listening..."):
            ...
            if "error" in res:
                ...
                if res["engine_id"] and res["engine_id"] in engine_list:
                    st.session_state.selected_engine = res["engine_id"]

# --- DATA PREP FOR SELECTED ENGINE ---
engine_df = df[(df['engine_id'] == st.session_state.selected_engine) & 
               (df['cycle'] <= st.session_state.current_cycle)]
latest_window = engine_df.tail(30)
service_hist = get_service_history(st.session_state.selected_engine)
...
# --- TAB 1: DEEP DIVE ---
with tab1:
    st.info(f"📋 **MRO Service History (SQL):** {service_hist}")
    ...
    with col_c2:
        ...
        gauge = go.Figure(go.Indicator(
            mode = "gauge+number",
            value = status['rul_prediction'],
            domain = {'x': [0, 1], 'y': [0, 1]},
            title = {'text': "Cycles Remaining"},
            gauge = {
                'axis': {'range': [0, 150]},
                'bar': {'color': "darkblue"},
                'steps': [
